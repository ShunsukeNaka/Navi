...
class YouTubeChatReader:
    ...
    async def poll(self) -> tuple[list[ChatMessage], float]:
        """
        ライブチャットを1回ポーリングしてメッセージリストと次回待機秒数を返す。

        Returns:
            (messages, wait_sec):
                messages  - フィルタ済みメッセージのリスト（通常コメント＋スーパーチャット）
                wait_sec  - 次回ポーリングまでの待機秒数（APIの推奨値、最低5秒）
        """
        if self._live_chat_id is None:
            raise RuntimeError("initialize() を先に呼び出してください")

        messages, polling_interval_ms, next_token = await self._fetch_messages(
        ...
        return messages, wait_sec

    async def stream_comments(self) -> AsyncIterator[ChatMessage]:
        """
        無限ループでポーリングし、新着メッセージがあればランダムに1件 yield する。
        スーパーチャットは通常コメントより優先して選択される。

        - メッセージなし → 待機のみ
        - スーパーチャットあり → スーパーチャットを優先してランダムに1件選択
        - 通常コメントのみ → ランダムに1件選択して yield
        ...
        while True:
            try:
                messages, wait_sec = await self.poll()
                retry_count = 0  # 成功したらリセット

                if messages:
                    # スーパーチャットを優先して選択
                    superchats = [m for m in messages if m.is_superchat]
                    yield random.choice(superchats) if superchats else random.choice(messages)
    ...
    async def _fetch_messages(
        ...
        return messages, polling_interval_ms, next_token

    def _extract_messages(self, items: list[dict]) -> list[ChatMessage]:
        """
        items から通常コメントとスーパーチャットを抽出する。

        除外対象:
          - 既取得コメント（同一IDへの重複反応防止）
          - スーパーステッカー（superStickerEvent）
          - Bot コメント（authorDetails.isChatBot == True）
          - 空テキスト（スーパーチャットはコメントなしでも含める）
        """
        result = []
        for item in items:
            ...
            author_name: str = author.get("displayName", "名無し")

            if msg_type == "textMessageEvent":
                text: str = snippet.get("textMessageDetails", {}).get("messageText", "").strip()
                if text:
                    self._seen_ids.add(msg_id)
                    result.append(ChatMessage(text=text, author=author_name))

            elif msg_type == "superChatEvent":
                details = snippet.get("superChatDetails", {})
                amount: str = details.get("amountDisplayString", "")
                text = details.get("userComment", "").strip()
                self._seen_ids.add(msg_id)
